...
contract DLLAction {
    ...
    enum Role { None, Attacker, Defender, Rewriter, Scanner, Measurer, Loader }
    ...
    event LibraryLoaded(address indexed user, string word, string newMetadata);
    ...
    function initializeWord(string memory _word, uint256 _initialAttackCount, uint256 _initialDefendCount, string memory _initialMetadata) public onlyOwner {
        ...
        emit LibraryLoaded(msg.sender, _word, _initialMetadata);
    }
    ...
    function loadLibrary(string memory _word, string memory _newMetadata) public onlyAuthorizedForWord(_word, Role.Loader) {
        require(initializedWords[_word], "Word not initialized");
        require(bytes(_newMetadata).length > 0, "Metadata cannot be empty");
        wordMetadata[_word] = _newMetadata;
        emit LibraryLoaded(msg.sender, _word, _newMetadata);
    }
