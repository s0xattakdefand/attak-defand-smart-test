...
contract DDNSAction {
    ...
    event MetadataUpdated(address indexed user, string word, string newMetadata);
    ...
    function initializeWord(string memory _word, uint256 _initialAttackCount, uint256 _initialDefendCount, string memory _initialMetadata) public onlyOwner {
        ...
        emit MetadataUpdated(msg.sender, _word, _initialMetadata);
    }
    ...
    function updateMetadata(string memory _word, string memory _newMetadata) public onlyAuthorizedForWord(_word, Role.Updater) {
        require(initializedWords[_word], "Word not initialized");
        require(bytes(_newMetadata).length > 0, "Metadata cannot be empty");
        wordMetadata[_word] = _newMetadata;
        emit MetadataUpdated(msg.sender, _word, _newMetadata);
    }
