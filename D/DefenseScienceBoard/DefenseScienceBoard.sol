...
/**
 * @title DefenseScienceBoardRepository
 * @notice On‐chain repository for Defense Science Board data with category‐based
 *         read/write permissions and full audit logging.
 */
contract DefenseScienceBoardRepository {
    struct Record {
        string  data;
        address author;
        uint256 timestamp;
    ...
    modifier onlyCategoryAdmin(string calldata category) {
        require(_categories[category].exists, "DSB: category not found");
        require(_categories[category].admin == msg.sender, "DSB: caller is not category admin");
        _;
    }

    modifier onlyWriter(string calldata category) {
        require(_categories[category].exists, "DSB: category not found");
        require(_categories[category].writers[msg.sender], "DSB: write access denied");
        _;
    }
    ...
    function createCategory(string calldata category) external {
        ...
        require(!c.exists, "DSB: category already exists");
        c.exists = true;
        c.admin = msg.sender;
    ...
    {
        require(newAdmin != address(0), "DSB: new admin zero address");
        ...
        )
    {
        ...
        require(bytes(r.data).length != 0, "DSB: record not found");
