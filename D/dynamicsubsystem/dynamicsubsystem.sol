...
contract DynamicSubsystemAction {
    ...
    enum Role { None, Attacker, Defender, Rewriter, Scanner, Measurer, Configurer }
    ...
    event SubsystemConfigured(address indexed user, string word, string newMetadata);
    ...
    function initializeWord(string memory _word, uint256 _initialAttackCount, uint256 _initialDefendCount, string memory _initialMetadata) public onlyOwner {
        ...
        emit SubsystemConfigured(msg.sender, _word, _initialMetadata);
    }
    ...
    function configureSubsystem(string memory _word, string memory _newMetadata) public onlyAuthorizedForWord(_word, Role.Configurer) {
        require(initializedWords[_word], "Word not initialized");
        require(bytes(_newMetadata).length > 0, "Metadata cannot be empty");
        wordMetadata[_word] = _newMetadata;
        emit SubsystemConfigured(msg.sender, _word, _newMetadata);
    }
