...
// DNSSEC-Aware Name Server smart contract
contract DNSSECAwareNameServer {
    ...
    // Authorized resolvers for record queries and verification
    ...
    event RecordSet(string indexed domain, string subdomain, RecordType recordType, bytes data, bytes signature, uint256 timestamp);
        ...
        onlyDomainOwner(domain) 
    {
        ...
        bytes32 dataHash = keccak256(abi.encodePacked(domain, subdomain, recordType, data, block.timestamp));
        ...
        emit RecordSet(domain, subdomain, recordType, data, signature, block.timestamp);
    }

    // Resolve and verify a signed record
    function resolveAndVerify(string memory domain, string memory subdomain) 
        external 
        onlyResolver 
        returns (RecordType, bytes memory, bool, uint256) 
    {
        require(domains[domain].owner != address(0), "Domain not registered");
        require(block.timestamp <= domains[domain].expiryTime, "Domain expired");
        ...
        // Verify the signature
        bytes32 dataHash = keccak256(abi.encodePacked(domain, subdomain, record.recordType, record.data, record.timestamp));
        address signer = recoverSigner(dataHash, record.signature);
        bool isValid = signer == domains[domain].owner;

        emit RecordVerified(domain, subdomain, isValid);
        return (record.recordType, record.data, isValid, record.timestamp);
    }
