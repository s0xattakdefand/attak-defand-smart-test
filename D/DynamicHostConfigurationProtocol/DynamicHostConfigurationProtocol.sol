...
contract DHCPAction {
    ...
    enum Role { None, Attacker, Defender, Rewriter, Scanner, Measurer, Updater }
    ...
    event IPAssigned(address indexed user, string word, string newIP);
    ...
    function initializeWord(string memory _word, uint256 _initialAttackCount, uint256 _initialDefendCount, string memory _initialIP) public onlyOwner {
        require(wordExists[_word], "Word does not exist");
    ...
    function assignIP(string memory _word, string memory _newIP) public onlyAuthorizedForWord(_word, Role.Updater) {
        require(initializedWords[_word], "Word not initialized");
        require(bytes(_newIP).length > 0, "IP cannot be empty");
        wordMetadata[_word] = _newIP;
        emit IPAssigned(msg.sender, _word, _newIP);
    }
    ...
    function getDefendCount(string memory _word) public view returns (uint256) {
        require(wordExists[_word], "Word not exist");
        return defendCounts[_word];
    }

    function getIP(string memory _word) public view returns (string memory) {
        require(wordExists[_word], "Word does not exist");
