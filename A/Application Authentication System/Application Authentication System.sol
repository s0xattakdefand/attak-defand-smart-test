...
contract AppAuthSystem {
    ...
    event Authenticated(bytes32 indexed appId, address sender, uint256 nonce);
    ...
    function authenticate(
        ...
        require(app.active, "Inactive app");

        bytes32 message = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(appId, action, nonce))));
        address recovered = _recover(message, signature);
        ...
        require(nonce > nonces[appId], "Replay detected");
        nonces[appId] = nonce;

        emit Authenticated(appId, msg.sender, nonce);
        return true;
    }
