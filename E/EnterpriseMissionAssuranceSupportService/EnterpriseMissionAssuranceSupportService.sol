...
/**
 * SimpleRBAC
 * - Minimal role-based access control (self-contained, no external deps)
 * - DEFAULT_ADMIN_ROLE is admin of itself and can set role admins
 */
contract SimpleRBAC {
    ...
    function _setupRole(bytes32 role, address account) internal {
        // For constructor bootstrapping; no admin checks
        if (!_roles[role][account]) {
            _roles[role][account] = true;
            emit RoleGranted(role, account, msg.sender);
        }
